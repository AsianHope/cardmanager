...
Images = new FS.Collection("images", {
    stores: [
      ...
      new FS.Store.FileSystem("image", {path:base_path+"/public/static"})
    ]
});


Zipper = new FS.Collection("zip", {
  stores: [new FS.Store.FileSystem("zip", {path:base_path+"/public/Zips"})]
});
...
AdminConfig = {
  collections: {
    Cards: {
      ...
      tableColumns: [
        //collection helpers currently don't support filtering. when the package is updated can use 'link()',
...
Cards.helpers({
  link: function() {
    if (this._id) {
	  var barcode = (Cards.find({_id: this._id }).fetch()[0] != undefined) ? Cards.find({_id: this._id }).fetch()[0].barcode : 'barcode-error';
